<div class="row">
    <div class="col-sm-12 col-lg-6 mt-2">
        <div class="card mt-4 h-100">
            ...
            <div class="card-body">
                <form action="/update_module" method="POST">
                    @csrf
                    <input type="hidden" name="module_id" value="{{$module->id}}">
                    <div class="row g-3">
                        <div class="col">
                            <label for="name">Module Name:</label>
                            <input type="text" name="name" id="class_name" class="form-control" value="{{$module->name}}" required>
                        </div>
                        <div class="col">
                    ...
                    <textarea name="description" id="description" cols="20" rows="5" class="form-control">{{$module->description}}</textarea>

                    <div class="d-flex justify-content-evenly">
                        <div class="form-outline datepicker mt-3" data-mdb-format="yyyy-mm-dd">
                            <input type="text" name="available_on" id="available_on" class="form-control" value="{{$module->available_on}}">
                            <label for="available_in" class="form-label">Available On:</label>
                        </div>
                        <div class="form-outline datepicker mt-3" data-mdb-format="yyyy-mm-dd">
                            <input type="text" name="not_available" id="unavailable_on" class="form-control" value="{{$module->not_available}}">
                            <label for="unavailable_in" class="form-label">Unavailable On:</label>
                        </div>
                    </div>
                    <button type="submit" class="btn-primary btn mt-4 w-100">Update</button>
                </form>
            </div>
        </div>
    </div>
                    ...
                    <div id="QuizTab" class="tab-pane fade" role="tabpanel" aria-labelledby="QuizTab">
                        <div class="d-flex justify-content-between mb-3">
                            <div class="align-self-start">
                                <div class="input-group">
                                    @can('CreateContent')
                                        <button class="btn btn-primary rounded" data-mdb-toggle="modal" data-mdb-target="#createModuleQuizModal">Create Quiz</button>
                                    @endcan
                                </div>
                            </div>
                        </div>
